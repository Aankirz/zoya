...
| D16 | 2026-09-14 | **Sarvam AI is not the main voice.** Nova 2 Sonic stays primary; Sarvam Bulbul v3 is only an optional fallback voice (if it wins the Phase 2 A/B) and a post-hackathon path to more Indian languages | Sonic listens, thinks and speaks in one interruptible stream with native Strands support; a Sarvam STT→LLM→TTS pipeline adds latency and code, and voice isn't where AWS credits go | §7.4, §20 |
